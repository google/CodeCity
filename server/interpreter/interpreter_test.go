...
func TestInterpreterSimple(t *testing.T) {
	var tests = []struct {
		...
		{"var v; var f = function() {v = 49}; f(); v", simpleFunctionExpression, data.Number(49)},
		{"var v; var f = function(x) {v = x}; f(50); v", fExpWithParameter, data.Number(50)},
		{"(function(x){return x;})(51)", functionWithReturn, data.Number(51)},
		{"(function(){try {return true;} finally {return false;}})()", multipleReturn, data.Boolean(false)},
		{"var f=function(){throw 26;};try{f()}catch(e){e*2;}", throwCatch, data.Number(52)},
		...
		{"var a=57;do{a++;break;a++}while(false);a", breakDoWhile, data.Number(58)},
		{"foo:break foo", selfBreak, data.Undefined{}},
		{"var a=6;foo:{try{a*=10;break foo}finally{a--}};a", breakWithFinally, data.Number(59)},
		{"var a=59;do {try{continue}finally{a++}}while(false);a", continueWithFinally, data.Number(60)},
		{"var a=0;while(a++<60){try{break}finally{continue}};a", breakWithFinallyContinue, data.Number(61)},
		{"(function(){var i=0;while(i++<61){try{return 42}finally{continue}}return i})()", returnWithFinallyContinue, data.Number(62)},
		{`63||"foo"`, orTrue, data.Number(63)},
		{"false||64", orFalse, data.Number(64)},
		{"({})&&65", andTrue, data.Number(65)},
		{"0&&65", andFalse, data.Number(0)},
		{"var t=0;for(var i=0;i<12;i++){t+=i};t", forTriangular, data.Number(66)},
		{"var x=0, a={a:60,b:3,c:4}for(var i in a){x+=a[i]};x", forIn, data.Number(67)},
		{`var x=1,o={foo:"bar"},a={a:2,b:2,c:17};for(o.foo in a){x*=a[o.foo]};x`, forInMemberExp, data.Number(68)},
		{"var x=0,o={},f=function(){x+=20;return o},a={a:2,b:3,c:4}for(f().foo in a){x+=a[o.foo]};x", forInMembFunc, data.Number(69)},
		{"var o={f:function(){return this.foo},foo:70};o.f()", methodCall, data.Number(70)},
		{"var o={f:function(){return this}};var g=o.f;g()", demethodedCall, data.Undefined{}},
		...
		{`try{typeof undeclaredVar}catch(e){"whoops!"}`, unaryTypeofUndeclared, data.String("undefined")},
...
// }
// => "undeclared"
const unaryTypeofUndeclared = `{"type":"Program","start":0,"end":55,"body":[{"type":"TryStatement","start":0,"end":55,"block":{"type":"BlockStatement","start":4,"end":30,"body":[{"type":"ExpressionStatement","start":8,"end":28,"expression":{"type":"UnaryExpression","start":8,"end":28,"operator":"typeof","prefix":true,"argument":{"type":"Identifier","start":15,"end":28,"name":"undeclaredVar"}}}]},"handler":{"type":"CatchClause","start":31,"end":55,"param":{"type":"Identifier","start":37,"end":38,"name":"e"},"guard":null,"body":{"type":"BlockStatement","start":40,"end":55,"body":[{"type":"ExpressionStatement","start":44,"end":53,"expression":{"type":"Literal","start":44,"end":53,"value":"whoops!","raw":"\"whoops!\""}}]}},"guardedHandlers":[],"finalizer":null}]}`
