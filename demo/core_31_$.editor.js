...
$.editor.page.jssp = [
  ...
  '}',
  'var isVerb = (obj.hasOwnProperty && obj.hasOwnProperty(key) && typeof obj[key] === \'function\') && obj[key].verb ? \'checked\' : \'\';',
  ...
  '  <div><input type="checkbox" name="isVerb" id="isVerb" <%= isVerb %> onclick="changeVerb(); changed()">',
  '    <label for="isVerb">Verb:</label>',
  '    <input name="verb" id="verb" value="<%= verb %>" placeholder="name">',
  '    <select name="dobj" id="dobj" onchange="changed()"><% for (var i = 0; i < objOpts.length; i++) {%>',
  '      <option <%= dobj === objOpts[i] ? \'selected\' : \'\' %>><%= objOpts[i] %></option>',
  '    <% } %></select>',
  '    <select name="prep" id="prep" onchange="changed()"><% for (var i = 0; i < $.utils.command.prepositionOptions.length; i++) {%>',
  '      <option <%= prep === $.utils.command.prepositionOptions[i] ? \'selected\' : \'\' %>><%= $.utils.command.prepositionOptions[i] %></option>',
  '    <% } %></select>',
  '    <select name="iobj" id="iobj" onchange="changed()"><% for (var i = 0; i < objOpts.length; i++) {%>',
