...
(function () {
  ...
  pot.getSvgText = function() {
    return this.stages[this.stage];
  };
  pot.plant = function(cmd) {
  ...
  pot.water = function(cmd) {
    user.narrate('You water ' + this.name + '.');
    if (user.location) {
      user.location.narrate(user.name + ' waters ' + this.name + '.');
    }
  ...
  thrower.fire = function(cmd) {
    var json = {
      type: 'iframe',
      url: '/static/flamethrower.html'
    };
    user.location.tellAll(json);
    if (cmd.iobj.seed) {
      cmd.iobj.seed = null;
    }
